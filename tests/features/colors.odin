...
create_color :: proc(t: ^testing.T) {
	...
	result := tuple == [3]f32{-0.5, 0.4, 1.7} 
...
add_color_and_color :: proc(t: ^testing.T) {
	...
	testing.expectf(t, tuples.color_equals(result, [3]f32{1.6, 0.7, 1.0}), "color addition was not correct, got %f, %f, %f", result.r, result.g, result.b)
}
...
subtract_color_and_color :: proc(t: ^testing.T) {
	...
	testing.expect(t, tuples.color_equals(result, [3]f32{0.2, 0.5, 0.5}), "color subtraction was not correct")
}

@(test)
multiply_color_by_scalar :: proc(t: ^testing.T) {
	c1 := tuples.color(0.2, 0.3, 0.4)
	expected := [3]f32{0.4, 0.6, 0.8}
